...
export default function TataAigInsurance() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Tata AIG General Insurance | Shamsuddin Ratnani";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute('content', "Protect your vehicle, home, travel, and business with comprehensive Tata AIG insurance solutions. Enjoy cashless claims, roadside assistance, and reliable assets protection.");
    }
  }, []);

  const plans = (agentConfig?.plans || []).filter(
    plan => !plan.hidden && plan.provider && plan.provider.toLowerCase().includes('tata')
  );
  ...
  const handleEnquire = (title) => {
    handleCloseModal();
    navigate(`/?tab=insurance&plan=general#contact`);
  };

  const scrollToPlans = () => {
    const el = document.getElementById('tata-plans-grid');
    if (el) {
      window.scrollTo({
  ...
  return (
    <div className="premium-partner-page tata-insurance-page">
      {/* Premium Hero Banner */}
      <section 
        className="partner-hero" 
        style={{ backgroundImage: `url('https://images.unsplash.com/photo-1542442247-eefaf8217d96?auto=format&fit=crop&q=80&w=1920')` }}
      >
        ...
        <div className="partner-hero-container">
          <div className="partner-hero-content">
            ...
            <h1 className="partner-hero-title">
              TATA AIG <span className="highlight-gold">Insurance</span>
            </h1>
            <p className="partner-hero-subtitle">
              Protect your vehicle, home, travel, and business with comprehensive Tata AIG insurance solutions.
            </p>
            <button className="partner-hero-btn" onClick={scrollToPlans}>
              VIEW TATA AIG PLANS <ArrowDown size={18} />
            </button>
          </div>
        </div>
      </section>
      ...
      {/* Plans Section */}
      <section className="partner-plans-section" id="tata-plans-grid">
        <div className="container">
          <div className="partner-section-header">
            <span className="partner-section-subtitle">Official Partner Products</span>
            <h2 className="partner-section-title">EXCLUSIVE TATA AIG PLANS</h2>
          </div>
          ...
          {plans.length === 0 ? (
            <div className="no-results glass-panel" style={{ textAlign: 'center', padding: '40px' }}>
              ...
              <h3>No Tata AIG Plans Loaded</h3>
